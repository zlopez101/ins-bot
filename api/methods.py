...
async def _raise_for_status(response: ClientResponse, response_object) -> Tuple[dict, str]:
...
async def _list_records(session: ClientSession, url: str, response_object: object, params: dict = None, pageSize: int=5, offset: str = None):
    if params:
        # only update params with key,value pairs that have a value
        params.update({k:v for k,v in zip(["pageSize", "offset"], [pageSize, offset]) if v})
    async with session.get(url, params=params) as resp:
        return await _raise_for_status(resp, response_object)

async def _retrieve_record(session: ClientSession, url: str, id: str, response_object: object):
    async with session.get(url + id) as resp:
        return await _raise_for_status(resp, response_object)
    
async def _patch_record(session: ClientSession, url: str, id: str, updates: Dict[str, str]):
